...
function ExpensesScreen({ params }) {
    ...
    // Get Budget Info and Expenses Together
    const getBudgetInfo = async () => {
    ...
    // Delete Budget
    const deleteBudget = async () => {
        await db.delete(Expenses).where(eq(Expenses.budgetId, paramId)).returning()

        const result = await db.delete(Budgets).where(eq(Budgets.id, paramId)).returning()
    ...
    return (
        <div>
            <h2 className='font-bold text-2xl p-4 flex justify-between items-center'>
                My Expenses
